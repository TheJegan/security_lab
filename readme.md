...
The environment to be deployed by Contoso is shown in figure 1.
...
*All usernames and passwords for virtual machines are set to ops / Starbucks1234*

Perform the following steps to initialise the lab environment:
- Follow class naming standards.
- Create 2 Resource groups: one for IAAS offerings, one for PAAS Offerings
- Create 2 Windows VMs based on Windows 2016 Datacenter.
- Create 2 Linux VMs based on Ubuntu 16.04 LTS. Option: generate your own SSH Key pair and use instaed of username/pass.
- Create a PAAS Webapp. Setup deployment from https://github.com/Araffe/Contoso-Web.git
- Create a storage account and a blob container. Upload several files (can be anything you want). Set the access policy to Container Anonymous Read access.
- Create a SQL Database in the same resource group as your PAAS Webapp. Initialize with the sample DB.

**PLEASE NOTE: Because this lab has been modified for Starbucks, some of the names have changed. What you are trying to accomplish remains the same though.
...
**1)** In the Azure portal, click on Security Center on the left hand menu. NOTE: Only one person in the subscription will need to do this!
...
First, we’ll create a few variables relating to the storage account, container, resource group and the name of our policy. **Note that you will need to replace the storage account name in the storageAccount variable with your own storage account name and resource group name. Please feel free to use the CLI if desired. You can look up the commands in the CLI 2.0 Docs.
...
**1)** In the Azure portal, navigate to the your-IaaS resource group. Click ‘Add’ and then search for ‘Network Security Group’ from the marketplace. Choose Network Security Group and then click ‘Create’.
...
**10)** Now that the NSG has been applied, let’s make sure we can still access our website. From the Azure portal, click on the ‘VM1-PIP’ resource within the ‘Contoso-IaaS’ resource group. Copy the IP address and then attempt to browse to it. You should still have access to the website. Remove all NSGs that are not being used anymore.
...
Rather than listing out all steps in this lab guide, please follow the steps on the documentation page to encrypt your first VM in your resource group.
...
In this exercise we will enable PIM for the tenant and then change a user from a 24/7 Global Administrator to an eligible user where they must respond to an MFA challenge to become a Global Administrator for 4 hours. We'll also view the audit log for PIM.
...
**10)** In the Global Administrator blade, click on any user.
...
**2)** Navigate to the resource group ‘your-PaaS’ and then select ‘Access Control (IAM)’.

**3)** Add a user as a Contributor Add > choose Contributor > <your user>.
...
**4)** In a separate browser sign into the Azure Portal as your user. 

**5)** Click on Resource Groups – Note that the user can only see the resources in the PaaS resource group as he does not have permissions to the Contoso-IaaS resources.
...
**3)** Click Add User > select any user you created > set the membership settings as followed, membership choose Direct > assignment start date Current day > assignment end date Current day + 1 month > type a justification for the change > Done
...
**1)** In the Azure portal, navigate to the your PaaS resource group and then click on Policies in the menu.
...
<pre lang="...">
New-azurermvm -resourcegroupname "your-paas" -name "policy-test-VM"  -imagename "UbuntuLTS"
 </pre>
